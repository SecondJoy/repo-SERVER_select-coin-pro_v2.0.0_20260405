...
from config import job_num, raw_data_path
...
from cta_core.select_coin import calc_factors, select_coins, concat_select_results, process_select_results, \
    agg_multi_strategy_ratio, calc_cross_sections
from cta_core.eval_config import CONFIG as EVAL_CONFIG
from cta_core.evaluate_picks import run_evaluation_task
...
def step3_calc_factors(conf: BacktestConfig):
    ...
    :return:
    """
    divider('时序因子计算', with_timestamp=False, sep='-')
    s_time = time.time()
    calc_factors(conf)
    logger.ok(f'完成计算时序因子，花费时间：{time.time() - s_time:.2f}秒')

    # 计算截面因子
    divider('截面因子计算', with_timestamp=False, sep='-')
    s_time = time.time()
...
def step7_evaluate_picks(conf: BacktestConfig):
    """
    评估选币结果
    :param conf:
    :return:
    """
    divider('评估选币结果', with_timestamp=False, sep='-')
    logger.info('开始评估选币结果...')
    s_time = time.time()

    # Update config with dynamic paths
    result_folder = conf.get_result_folder()
    eval_config = EVAL_CONFIG.copy()
    eval_config['picks_path'] = str(result_folder)  # Point to the result folder
    eval_config['picks_pattern'] = 'final_select_results.pkl'  # Ensure it picks the right file
    eval_config['out_dir'] = str(result_folder / 'evl_output')  # Output inside result folder

    try:
        run_evaluation_task(eval_config)
        logger.ok(f'完成评估选币结果，花费时间：{time.time() - s_time:.2f}秒')
        logger.info(f'评估报告已生成至: {eval_config["out_dir"]}')
    except Exception as e:
        logger.error(f'评估选币结果失败: {e}')
        import traceback
...
def simu_re_timing(conf: BacktestConfig, df_spot_ratio, df_swap_ratio, pivot_dict_spot, pivot_dict_swap):
    ...
    if conf.timing.impl_flags.get('dynamic_leverage_for_dataframe', False):
        leverage = conf.timing.get_dynamic_leverage_for_dataframe(account_df)
...
def find_best_params(factory: BacktestConfigFactory, show_heat_map=False):
    ...
    calc_factors(conf)  # 执行时序因子计算
    logger.ok(f'完成计算时序因子，花费时间：{time.time() - s_time:.3f}秒，累计时间：{(time.time() - r_time):.3f}秒')

    # 计算截面因子
    s_time = time.time()
    logger.info(f'截面因子计算...')
    calc_cross_sections(conf)  # 执行截面因子计算
