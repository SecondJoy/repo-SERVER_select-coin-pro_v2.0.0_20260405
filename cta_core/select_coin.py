...
from cta_core.model.strategy_config import StrategyConfig
...
* 需要计算币种数量：{len(candle_df_list)} 个''')
    ...
    # 清理 cache 的缓存
    all_kline_pkl = conf.get_result_folder() / 'all_factors_kline.pkl'
    all_kline_pkl.unlink(missing_ok=True)

    all_kline_full_pkl = conf.get_result_folder() / 'all_factors_kline_full.pkl'
    ...
    for shard_index in shards:
        ...
        if config.job_num > 1:
            with ProcessPoolExecutor(max_workers=config.job_num) as executor:
            ...
            # Serial execution to avoid OOM and overhead
            for candle_idx, candle_df in tqdm(enumerate(candle_df_list), total=len(candle_df_list), desc='🧮 时序因子计算(串行)'):
                idx, factor_df = process_candle_df(candle_df.copy(), conf, factor_col_name_list, candle_idx)
                all_factor_df_list[idx] = factor_df
        ...
        if not all_kline_pkl.exists():
            ...
            # 修改：将K线缓存保存到配置结果目录
            all_kline_df.to_pickle(conf.get_result_folder() / 'all_factors_kline.pkl')
        ...
        if not all_kline_full_pkl.exists() and conf.has_section_factor:
            ...
            # 修改：将K线缓存保存到配置结果目录
            all_kline_full_df.to_pickle(conf.get_result_folder() / 'all_factors_kline_full.pkl')
        ...
        for factor_col_name in factor_col_name_list:
            # 修改：将缓存路径修改为配置的缓存路径，而不是固定的 data/cache
            # factor_pkl = get_file_path('data', 'cache', f'factor_{factor_col_name}.pkl', as_path_type=True)
            factor_pkl = conf.get_result_folder() / f'factor_{factor_col_name}.pkl'
            ...
            if conf.has_section_factor:
                ...
                factor_full_pkl = conf.get_result_folder() / f'factor_full_{factor_col_name}.pkl'
...
def process_factor_df(factor_col_name, result_folder):
    # 准备所有时序因子数据
    factor_path = result_folder / f'factor_full_{factor_col_name}.pkl'
...
def load_all_factors(conf: BacktestConfig):
    ...
    if config.job_num > 1:
        with ProcessPoolExecutor(max_workers=config.job_num) as executor:
            futures = [executor.submit(
                process_factor_df, factor_col_name, conf.get_result_folder()
        ...
        for factor_col_name in tqdm(conf.section_depend_factor_col_name_list, desc='✂️ 裁切时序因子数据(串行)'):
            factor_col_name, kline_with_factor_df = process_factor_df(factor_col_name, conf.get_result_folder())
            if not kline_with_factor_df.empty:
                factor_df[factor_col_name] = kline_with_factor_df
...
def calc_select_factor_rank(df, factor_column='因子', ascending=True):
    ...
    :return:                计算排名后的数据框
    """
    # 如果存在quote_volume列，则将其作为第二排序键，避免因子值相同时，随机选币，而导致选币结果不稳定，这里可以自行选择其他因子作为第二排序键
    sort_cols = [factor_column]
    sort_orders = [ascending]
    
    if 'quote_volume' in df.columns:
        sort_cols.append('quote_volume')
        sort_orders.append(False) # 成交额总是从大到小排序（优先选成交额大的）
    
    # 对数据进行排序
    df.sort_values(by=sort_cols, ascending=sort_orders, inplace=True)
    
    # 生成排名（简单地使用当前的行号作为排名，因为已经排好序了）
    # 注意：这里需要按时间分组生成排名
    df['rank'] = df.groupby('candle_begin_time').cumcount() + 1
...
def ensure_cta_compatibility(stg_conf):
    """
    动态补丁：确保 stg_conf 具有 CTA 相关的属性和方法
    """
    # 动态补丁：确保有 cta_signal_list 属性
    if not hasattr(stg_conf, 'cta_signal_list'):
        stg_conf.cta_signal_list = []
    
    # 动态补丁：确保有 filter_by_signal 方法
    if not hasattr(stg_conf, 'filter_by_signal'):
        from cta_core.model.strategy_config import StrategyConfig as NewStrategyConfig, filter_common
        # 绑定方法
        stg_conf.filter_by_signal = NewStrategyConfig.filter_by_signal.__get__(stg_conf, stg_conf.__class__)
        stg_conf.filter_signal = NewStrategyConfig.filter_signal.__get__(stg_conf, stg_conf.__class__)
...
def select_coins_by_strategy(factor_df, stg_conf: StrategyConfig):
    ...
    :return: 选币数据
    """
    ensure_cta_compatibility(stg_conf)
    ...
    """
    4.3.5 CTA 信号过滤
    在调用 select_long_and_short_coin 之前，显式添加 CTA 信号过滤步骤
    """
    long_df, short_df = stg_conf.filter_by_signal(long_df, short_df)
...
def process_strategy(stg_conf: StrategyConfig, result_folder: Path):
    ...
    # 准备选币用数据
    # 修改：从 result_folder 读取 k线数据
    factor_df = pd.read_pickle(result_folder / 'all_factors_kline.pkl')
    # 时序因子
    for factor_col_name in stg_conf.factor_columns:
        # 修改：从 result_folder 读取因子数据
        factor_df[factor_col_name] = pd.read_pickle(result_folder / f'factor_{factor_col_name}.pkl')
    ...
    ** 策略选币 **
    ...
    if config.job_num > 1:
        ...
        for stg in conf.strategy_list:
            process_strategy(stg, result_folder)


def select_coins(confs: Union[BacktestConfig, List[BacktestConfig]], multi_process=True):
    ...
    if config.job_num > 1:
        ...
        for conf in tqdm(confs, total=len(confs), desc='选币(串行)'):
            select_coin_with_conf(conf, is_multi, is_silent)
...
def process_select_results(conf: BacktestConfig) -> pd.DataFrame:
    ...
    if not all_select_result_df[cond1 & cond2].empty:
        ...
        all_kline_df = pd.read_pickle(conf.get_result_folder() / 'all_factors_kline.pkl')
