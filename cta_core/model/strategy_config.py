...
class StrategyConfig:
    ...
    long_select_coin_num: Union[int, float, tuple] = 0.1
    # 空头选币数量。1 表示做空一个币; 0.1 表示做空10%的币，'long_nums'表示和多头一样多的数量
    short_select_coin_num: Union[int, float, tuple, str] = 'long_nums'  # 注意：多头为0的时候，不能配置'long_nums'
    # 选币范围控制，
    # 默认为'both'，表示 <= 和 >=；'left' 表示 >= 和 <；'right' 表示 > 和 <=。
    # 也支持多空分离，比如 ['both', 'left']，表示多头是 both 模式，空头是 left 模式
    select_inclusive: Union[str, tuple] = 'right'
    ...
    # CTA 信号因子列表
    cta_signal_list: List[FilterFactorConfig] = field(default_factory=list)

    long_factor_list: List[FactorConfig] = field(default_factory=list)  # 多头选币因子
    short_factor_list: List[FactorConfig] = field(default_factory=list)  # 空头选币因子
    ...
    def factor_columns(self) -> List[str]:
        ...
        # 针对当前策略的 CTA 信号因子信息，整理之后的列名信息，并且缓存到全局
        for signal_factor in self.cta_signal_list:
            # 策略 CTA 信号因子最终在df中的列名
            factor_columns.add(signal_factor.col_name)  # 添加到当前策略缓存信息中
    ...
    def init(cls, index: int, file: DummyStrategy = None, **config):
        ...
        # 初始化 CTA 信号因子
        cta_signal_list = config.get('cta_signal_list', [])
        config['cta_signal_list'] = [FilterFactorConfig.init(item) for item in cta_signal_list]
...
- 空头选币设置(权重 {self.short_cap_weight * 100:.2f}%):
  ...
  * 后置过滤: {self.short_filter_list_post}"""
    ...
    def filter_signal(self, long_df, short_df, **kwargs) -> (pd.DataFrame, pd.DataFrame):
        """
        CTA 信号过滤逻辑，供策略文件覆盖
        默认逻辑：
        1. 如果配置了 cta_signal_list，则所有信号必须同时满足 (AND)
        """
        # 如果配置了 CTA 信号列表，使用通用过滤逻辑
        if self.cta_signal_list:
            # 对 long_df 进行过滤
            if not long_df.empty:
                long_signal_condition = filter_common(long_df, self.cta_signal_list)
                long_df = long_df[long_signal_condition].copy()
            
            # 对 short_df 进行过滤
            if not short_df.empty:
                short_signal_condition = filter_common(short_df, self.cta_signal_list)
                short_df = short_df[short_signal_condition].copy()
    ...
    def filter_by_signal(self, long_df, short_df):
        """
        执行信号过滤
        """
        # 如果策略文件中定义了 filter_signal，则优先使用（但通常 filter_signal 也是需要绑定到实例上的）
        # 这里我们假设 filter_signal 是 StrategyConfig 的方法，默认实现就是上面的逻辑
        # 如果用户想自定义复杂的信号逻辑（比如多因子组合判断），可以在策略文件中覆盖这个方法
        return self.filter_signal(long_df, short_df)
