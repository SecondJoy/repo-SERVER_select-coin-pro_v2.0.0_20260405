...
# pre_data_path is already set above
...
data_source_dict = {
    ...
    "coin-cap": ('load_coin_cap', 'E:/xbx/xbx_data/coin-cap',)
}
...
end_date = '2026-01-01 00:00:00'  # 回测结束时间
...
backtest_name = 'VMRatio'  # 回测的策略组合的名称。可以自己任意取。一般建议，一个回测组，就是实盘中的一个账户。  # VMRatio
strategy_list = load_strategy_by_backtest_name(backtest_name)
min_kline_num = 24  # 最少上市多久，不满该K线根数的币剔除，即剔除刚刚上市的新币。168：标识168个小时，即：7*24
black_list = ['BTC-USDT', 'ETH-USDT']  # 拉黑名单，永远不会交易。不喜欢的币、异常的币。例：LUNA-USDT, 这里与实盘不太一样，需要有'-'
white_list = []  # type: ignore[var-annotated] # 如果不为空，即只交易这些币，只在这些币当中进行选币。例：LUNA-USDT, 这里与实盘不太一样，需要有'-'
...
# ====================================================================================================
# job_num = max(os.cpu_count() - 1, 10)  # 回测并行数量
job_num = 10  # 回测并行数量
